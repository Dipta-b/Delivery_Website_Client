...
// Fix Leaflet icon paths (needed for many setups)
...
// FitBounds component: when coords change, fit map to show them all
...
const Coverage = () => {
    ...
    // ---- Full district data (your provided 64-ish items) ----
    // Note: Each object has latitude and longitude fields. We use [latitude, longitude]
    const districts = [
        {
            region: "Dhaka",
            district: "Dhaka",
    ...
    // filteredDistricts: if searchTerm is empty, this returns all districts
    const filteredDistricts = districts.filter((d) =>
        d.district.toLowerCase().includes(searchTerm.toLowerCase())
    );

    // Build coords array for FitBounds (use [lat, lng])
    const coords = filteredDistricts.map((d) => [d.latitude, d.longitude]);
    ...
    return (
        <div className="min-h-screen bg-base-200 flex flex-col items-center py-8 px-4">
            ...
            {/* Search */}
            <div className="form-control w-full max-w-md mb-6 flex gap-2">
                <input
                    type="text"
            ...
            {/* Map */}
            <div className="w-full max-w-6xl h-[640px] rounded-2xl overflow-hidden shadow-lg border border-base-300">
                <MapContainer
                ...
                >
                    ...
                    {/* Fit to show all current markers */}
                    <FitBounds coords={coords} />

                    {filteredDistricts.map((d, i) => (
                        <Marker key={`${d.district}-${i}`} position={[d.latitude, d.longitude]}>
                            <Popup>
                                <div className="max-w-xs">
                                    ...
                                    <p className="text-sm mb-2">
                                        <strong>Covered:</strong> {d.covered_area.join(", ")}
                                    </p>
                                    {d.flowchart ? (
                                        <img
                                            src={d.flowchart}
